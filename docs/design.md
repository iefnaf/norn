...
- `maxWorkRounds` bounds both the worker rounds of one Work attempt (§10.2) and the number of in-run conflict reworks of one Ticket (§12);
...
- blocked and ticket-scoped error outcomes park their ticket for the current run — except a ticket-scoped `integration-conflict`, which is re-queued for fresh Work in a later Wave of the same run at the advanced base, carrying the conflict as worker feedback; a Ticket may be reworked at most `maxWorkRounds` times per run, after which the conflict parks it with the ordinary ticket-scoped semantics;
...
type TimelineAnchor =
  ...
  | { kind: 'prefix'; timelineLength: number; prefixDigest: string }
...
type RunState = {
  ...
  reworks?: Record<string, TicketRework>
...
- If state is `running`, the same `configRevision` and `nornVersion` resume the run ID, parked set, accepted Map lineage, exact Wave queue, persisted retry counters, and in-run rework ledger.
...
1. If the Map is OPEN, revision and target still equal the checkpoint, and no close-then-reopen sequence occurred after `timelineAnchor`, Norn may retry the close protocol in §15. A detected reopen discards the old gates and restarts full completion rather than silently reclosing. If revision or target moved, Norn also discards the stale gates, releases the target lock before taking any repository control lock, and then adopts, restarts, or blocks according to the ordinary rules.
2. If the Map is CLOSED, Norn binds it to the checkpoint only when the current closing event is the authenticated actor's first close after `timelineAnchor` and no reopen follows it. It then completes the post-close reads. When revision and target still match, it seals or locates the `MapCompletionRecordV1` and validates it before terminalizing.
...
8. only when Map revision and target are unchanged, captures the exact current timeline boundary as `timelineAnchor` (event ID when available, otherwise a verified synthetic prefix), persists the `gated` `MapCompletionCheckpoint`, and closes the still-open Map;
