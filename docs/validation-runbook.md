...
- The `pi` on the Herdr server's `PATH` is at least 0.87.0 (`00-context.txt` records `pi --version`). The completion extension's settlement enforcement (#33) registers its `agent_before_settle` / `agent_settled` handlers, which older Pi builds neither dispatch nor typecheck against; on such a build a child that answers in prose idles until the invocation timeout again.
